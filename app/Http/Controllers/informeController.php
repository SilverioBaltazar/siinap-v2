...
class informeController extends Controller
{
    ...
    public function actionBuscarInforme(Request $request)
    {
        ...
        $bio   = $request->get('bio');    
        $regprogtrab = regProgtrabModel::orderBy('PERIODO_ID', 'ASC')
                       ->orderBy('FOLIO', 'ASC')
        ...
        //return view('sicinar.programatrabajo.verProgtrab', compact('nombre','usuario','regiap','regumedida','reganios','regperiodos','regmeses','regdias','regprogtrab'));
        return view('sicinar.informelabores.verInformes', compact('nombre','usuario','regiap','reganios','regperiodos','regmeses','regdias','regprogtrab','totactivs'));
    }
    ...
    public function actionVerInformes(){
        ...
        $regprogdtrab = regProgdtrabModel::select('FOLIO','PARTIDA','PERIODO_ID','IAP_ID','DFECHA_ELAB',
        ...
        if(session()->get('rango') !== '0'){  
            $totactivs  =regprogtrabModel::join('JP_PROGRAMA_DTRABAJO','JP_PROGRAMA_DTRABAJO.FOLIO', '=', 
                         ...
                         ->groupBy('JP_PROGRAMA_ETRABAJO.PERIODO_ID','JP_PROGRAMA_ETRABAJO.FOLIO')
                         ->get();                   
            $regprogtrab=regProgtrabModel::select('FOLIO','PERIODO_ID','IAP_ID','FECHA_ELAB','FECHA_ELAB2',
        ...
        if($regprogtrab->count() <= 0){
            toastr()->error('No existen programas de trabajo.','Lo siento!',['positionClass' => 'toast-bottom-right']);
            //return redirect()->route('nuevoprogtrab');
        }
        //return view('sicinar.programatrabajo.verProgtrab',compact('nombre','usuario','regiap','regumedida','reganios','regperiodos','regmeses','regdias','regprogtrab'));
        return view('sicinar.informelabores.verInformes',compact('nombre','usuario','regiap','reganios','regperiodos','regmeses','regdias','regprogtrab','regprogdtrab','totactivs')); 
    ...
    public function actionNuevoProgtrab(){
        ...
        $regprogtrab  = regProgtrabModel::select('FOLIO','PERIODO_ID','IAP_ID','FECHA_ELAB','FECHA_ELAB2',
                        ...
                        ->get();
        //dd($unidades);
        //return view('sicinar.programatrabajo.nuevoProgtrab',compact('regumedida','regiap','nombre','usuario','reganios','regperiodos','regmeses','regdias','regprogtrab'));
        return view('sicinar.programatrabajo.nuevoProgtrab',compact('regiap','nombre','usuario','reganios','regperiodos','regmeses','regdias','regprogtrab'));                         
    }
    ...
    public function actionAltaNuevoProgtrab(Request $request){
        ...
        // *************** Validar duplicidad ***********************************/
        $duplicado = regProgtrabModel::where(['PERIODO_ID' => $request->periodo_id,'IAP_ID' => $request->iap_id])
                     ->get();
        if($duplicado->count() >= 1)
            ...
            $nuevoprogtrab->IAP_ID        = $request->iap_id;
    ...
    public function actionEditarProgtrab($id){
        ...
        //return view('sicinar.programatrabajo.editarProgtrab',compact('nombre','usuario','regiap','regumedida','reganios','regperiodos','regmeses','regdias','regprogtrab'));
        return view('sicinar.programatrabajo.editarProgtrab',compact('nombre','usuario','regiap','reganios','regperiodos','regmeses','regdias','regprogtrab'));
    ...
    public function actionExportInformePdf($id,$id2){
        ...
        if(session()->get('rango') !== '0'){                 
            ...
            $regprogtrab= regProgtrabModel::select('FOLIO','PERIODO_ID','IAP_ID','FECHA_ELAB','FECHA_ELAB2',
                        ...
                        'RESPONSABLE','ELABORO','AUTORIZO','OBS_1','OBS_2','STATUS_1','STATUS_2',
                        'FECREG','IP','LOGIN','FECHA_M','IP_M','LOGIN_M')
                   ->where(['FOLIO' => $id2, 'IAP_ID' => $arbol_id])
                   ->orderBy('PERIODO_ID','ASC')
                   ->orderBy('FOLIO','ASC')
                   ->get();            
        }                                       
        $regprogdtrab=regProgdtrabModel::join('JP_CAT_UNID_MEDIDA' ,'JP_CAT_UNID_MEDIDA.UMEDIDA_ID','=',
                                                                    'JP_PROGRAMA_DTRABAJO.UMEDIDA_ID')
                ->select('JP_PROGRAMA_DTRABAJO.PARTIDA', 
                         ...
                         'JP_PROGRAMA_DTRABAJO.MESC_01', 'JP_PROGRAMA_DTRABAJO.MESC_02', 'JP_PROGRAMA_DTRABAJO.MESC_03', 
                         'JP_PROGRAMA_DTRABAJO.MESC_04', 'JP_PROGRAMA_DTRABAJO.MESC_05', 'JP_PROGRAMA_DTRABAJO.MESC_06', 
                         'JP_PROGRAMA_DTRABAJO.MESC_07', 'JP_PROGRAMA_DTRABAJO.MESC_08', 'JP_PROGRAMA_DTRABAJO.MESC_09', 
                         'JP_PROGRAMA_DTRABAJO.MESC_10', 'JP_PROGRAMA_DTRABAJO.MESC_11', 'JP_PROGRAMA_DTRABAJO.MESC_12'
                            )
                ->selectRaw('(JP_PROGRAMA_DTRABAJO.MESP_01+JP_PROGRAMA_DTRABAJO.MESP_02+JP_PROGRAMA_DTRABAJO.MESP_03+
                              ...
                              JP_PROGRAMA_DTRABAJO.MESP_10+JP_PROGRAMA_DTRABAJO.MESP_11+JP_PROGRAMA_DTRABAJO.MESP_12)
                              META_PROGRAMADA')
                ->selectRaw('(JP_PROGRAMA_DTRABAJO.MESC_01+JP_PROGRAMA_DTRABAJO.MESC_02+JP_PROGRAMA_DTRABAJO.MESC_03+
                              JP_PROGRAMA_DTRABAJO.MESC_04+JP_PROGRAMA_DTRABAJO.MESC_05+JP_PROGRAMA_DTRABAJO.MESC_06+
                              JP_PROGRAMA_DTRABAJO.MESC_07+JP_PROGRAMA_DTRABAJO.MESC_08+JP_PROGRAMA_DTRABAJO.MESC_09+
                              JP_PROGRAMA_DTRABAJO.MESC_10+JP_PROGRAMA_DTRABAJO.MESC_11+JP_PROGRAMA_DTRABAJO.MESC_12)
                              META_REALIZADA')                
                ->where(  'JP_PROGRAMA_DTRABAJO.FOLIO'     ,$id2)
                ->orderBy('JP_PROGRAMA_DTRABAJO.PERIODO_ID','ASC')                   
                ->orderBy('JP_PROGRAMA_DTRABAJO.FOLIO'     ,'ASC')
                ->orderBy('JP_PROGRAMA_DTRABAJO.PARTIDA'   ,'ASC')
                ->get();    
        //dd('Llave:',$id,' llave2:',$id2);       
        if($regprogdtrab->count() <= 0){
            toastr()->error('No existen registros de informe de actividades del programa de trabajo.','Uppss!',['positionClass' => 'toast-bottom-right']);
            return redirect()->route('verProgtrab');
        }else{
            $pdf = PDF::loadView('sicinar.pdf.InformePdf',compact('nombre','usuario','regumedida','regiap','regprogtrab','regprogdtrab'));
            //$options = new Options();
            ...
            return $pdf->stream('Informe_Labores_anual-'.$id2);
        }
    }
    ...
    public function actionVerInformelab($id){
        ...
        if(session()->get('rango') !== '0'){           
            ...
            $regprogtrab=regProgtrabModel::select('FOLIO','PERIODO_ID','IAP_ID','FECHA_ELAB','FECHA_ELAB2',
                         ...
                         'RESPONSABLE','ELABORO','AUTORIZO','OBS_1','OBS_2','STATUS_1','STATUS_2',
                         'FECREG','IP','LOGIN','FECHA_M','IP_M','LOGIN_M')
                         ->where(['FOLIO' => $id, 'IAP_ID' => $arbol_id])
                         //->where('FOLIO',$id)            
                         //->where('IAP_ID',$arbol_id)            
                         ->orderBy('PERIODO_ID','ASC')
                         ->orderBy('IAP_ID','ASC')
        ...
        return view('sicinar.informelabores.verInformelab',compact('nombre','usuario','regiap','regumedida','reganios','regperiodos','regmeses','regdias','regprogtrab','regprogdtrab'));
    }
    ...
    public function actionNuevoProgdtrab($id){
        ...
        //dd($unidades);
        //return view('sicinar.programatrabajo.nuevoProgtrab',compact('regumedida','regiap','nombre','usuario','reganios','regperiodos','regmeses','regdias','regprogtrab'));
        return view('sicinar.programatrabajo.nuevoProgdtrab',compact('nombre','usuario','regiap','regumedida','regprogtrab','regprogdtrab'));   
    }
    ...
    public function actionAltaNuevoProgdtrab(Request $request){
        ...
        //else{  
            ...
            $nuevoprogdtrab->IAP_ID        = $request->iap_id;
    ...
    public function actionEditarInformelab($id,$id2){
        ...
        $regprogdtrab = regProgdtrabModel::select('FOLIO','PARTIDA','PERIODO_ID','IAP_ID','DFECHA_ELAB',
                        ...
                        ->first();
        if($regprogdtrab->count() <= 0){
            toastr()->error('No existen registros de actividades del informe de labores del programa de trabajo.','Lo siento!',['positionClass' => 'toast-bottom-right']);
            //return redirect()->route('nuevoprogtrab');
        }
        //return view('sicinar.programatrabajo.editarProgtrab',compact('nombre','usuario','regiap','regumedida','reganios','regperiodos','regmeses','regdias','regprogtrab'));
        return view('sicinar.informelabores.editarInformelab',compact('nombre','usuario','regiap','reganios','regperiodos','regmeses','regdias','regprogtrab','regprogdtrab','regumedida'));
    ...
    public function actionActualizarInformelab(informelabRequest $request, $id, $id2){
        ...
        if($regprogdtrab->count() <= 0)
            toastr()->error('No existe actividad del inorme de labores del programa de trabajo.','¡Por favor volver a intentar!',['positionClass' => 'toast-bottom-right']);
        else{        
            //********************** Actualizar ********************************/
            $regprogdtrab = regProgdtrabModel::where(['FOLIO' => $id, 'PARTIDA' => $id2])        
                           ->update([                
                'MESC_01'       => $request->mesc_01,
                'MESC_02'       => $request->mesc_02,
                'MESC_03'       => $request->mesc_03,
                'MESC_04'       => $request->mesc_04,
                'MESC_05'       => $request->mesc_05,
                'MESC_06'       => $request->mesc_06,
                'MESC_07'       => $request->mesc_07,
                'MESC_08'       => $request->mesc_08,
                'MESC_09'       => $request->mesc_09,
                'MESC_10'       => $request->mesc_10,
                'MESC_11'       => $request->mesc_11,
                'MESC_12'       => $request->mesc_12,

                'DOBS_2'        => substr(trim(strtoupper($request->dobs_2)),0,299),        
            ...
            toastr()->success('Informe de labor de la actividad del programa de trabajo actualizada.','¡Ok!',['positionClass' => 'toast-bottom-right']);
