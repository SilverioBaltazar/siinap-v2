...
class visitasController extends Controller
{
    ...
    public function actionBuscarVisita(Request $request)
    {
        ...
        return view('sicinar.agenda.verVisitas', compact('nombre','usuario','estructura','id_estructura','regiap','regperiodos', 'regdias', 'reghoras','regmeses','regminutos','regvisita','regentidades','regmunicipio'));
    }
    ...
    public function actionVerVisitas(){
        ...
        $regvisita    = regAgendaModel::select('VISITA_FOLIO','PERIODO_ID','MES_ID','DIA_ID','HORA_ID','NUM_ID',
                                             ...
                                             'VISITA_FECREGD'  ,'VISITA_FECREGV' ,'FECHA_M','IP_M' ,'LOGIN_M')
        ...
        return view('sicinar.agenda.verVisitas',compact('nombre','usuario','estructura','id_estructura','regmeses', 'reghoras','regminutos','regdias','regperiodos','regiap','regvisita','regentidades','regmunicipio'));
    }
    ...
    public function actionEditarVisita($id){
        ...
        $reghoras     = regHorasModel::select('HORA_ID','HORA_DESC')->get();   
        $regdias      = regDiasModel::select('DIA_ID','DIA_DESC')->get();   
        $regminutos   = regNumerosModel::select('NUM_ID','NUM_DESC')->orderBy('NUM_ID','DESC')->get();   
        $regperiodos  = regPfiscalesModel::select('PERIODO_ID', 'PERIODO_DESC')->get();        
        $regiap       = regIapModel::select('IAP_ID', 'IAP_DESC','IAP_STATUS')->get();       
        $regvisita    = regAgendaModel::select('VISITA_FOLIO','PERIODO_ID','MES_ID','DIA_ID','HORA_ID','NUM_ID',
                                               'IAP_ID','MUNICIPIO_ID',
                                             ...
                                             'VISITA_SPUB2','VISITA_FECREGP','VISITA_EDO','VISITA_STATUS2',
                                             'VISITA_AUDITADO1','VISITA_PUESTO1' ,'VISITA_IDENT1'  ,'VISITA_EXPED1',
                                             'VISITA_AUDITADO2','VISITA_PUESTO2' ,'VISITA_IDENT2'  ,'VISITA_EXPED2',
                                             'VISITA_AUDITADO3','VISITA_PUESTO3' ,'VISITA_IDENT3'  ,'VISITA_EXPED3',
                                             'VISITA_AUDITOR1' ,'VISITA_AUDITOR2','VISITA_AUDITOR3',
        ...
        return view('sicinar.agenda.editarVisita',compact('nombre','usuario','estructura','id_estructura','regmeses','reghoras','regdias','regminutos','regiap','regperiodos','regentidades','regmunicipio','regvisita'));
    ...
    public function actionActualizarVisita(visitaRequest $request, $id){
        ...
        if($regvisita->count() <= 0)
            ...
            ->update([                
                ...
                'VISITA_AUDITADO1'=> strtoupper($request->visita_auditado1),
                'VISITA_PUESTO1'  => strtoupper($request->visita_puesto1),
                'VISITA_IDENT1'   => strtoupper($request->visita_ident1),
                'VISITA_EXPED1'   => strtoupper($request->visita_exped1),
                'VISITA_AUDITADO2'=> strtoupper($request->visita_auditado2),
                'VISITA_PUESTO2'  => strtoupper($request->visita_puesto2),
                'VISITA_IDENT2'   => strtoupper($request->visita_ident2),
                'VISITA_EXPED2'   => strtoupper($request->visita_exped2),
                'VISITA_AUDITADO3'=> strtoupper($request->visita_auditado3),                
                'VISITA_AUDITOR1' => strtoupper($request->visita_auditor1),
                'VISITA_AUDITOR2' => strtoupper($request->visita_auditor2),

                'VISITA_TESTIGO1' => strtoupper($request->visita_testigo1),
                'VISITA_TESTIGO2' => strtoupper($request->visita_testigo2),

                'VISITA_CRITERIOS'=> strtoupper($request->visita_criterios),
                'VISITA_VISTO'    => strtoupper($request->visita_visto),
                'VISITA_RECOMEN'  => strtoupper($request->visita_recomen),
                'VISITA_SUGEREN'  => strtoupper($request->visita_sugeren),
            ...
            toastr()->success('Visita de diligencia actualizada en la agenda correctamente.','¡Ok!',['positionClass' => 'toast-bottom-right']);
        }
    ...
    public function actionActaVisitaPDF($id){
        ...
        $pdf = PDF::loadView('sicinar.pdf.actavisitaPDF', compact('nombre','usuario','regmeses','reghoras','regdias','regminutos','regperiodos','regiap','regvisita','regentidades','regmunicipio'));
        //$options = new Options();
        ...
        return $pdf->stream('DocumentoDeVisitaVerificacion');
    }
