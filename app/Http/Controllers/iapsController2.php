...
use App\Http\Requests\iaps2Request;
...
class iapsController2 extends Controller
{
  

    public function actionEditarIap2($id){
        ...
        return view('sicinar.iaps.editarIap2',compact('nombre','usuario','estructura','id_estructura','regiap'));

    }

    public function actionActualizarIap2(iaps2Request $request, $id){
        $nombre        = session()->get('userlog');
        $pass          = session()->get('passlog');
        if($nombre == NULL AND $pass == NULL){
            return view('sicinar.login.expirada');
