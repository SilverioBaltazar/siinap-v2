...
class usuariosController extends Controller
{
    ...
    public function actionLogin(usuarioRequest $request){
    	//dd($request->all());
        $existe = usuarioModel::select('LOGIN','PASSWORD','TIPO_USUARIO','CVE_DEPENDENCIA','CVE_ARBOL','STATUS_1')
        ...
        if($existe->count()>=1){
    		...
    		session(['userlog' => $request->usuario,'passlog' => $request->password,'usuario' => $usuario,'estructura' => $estructura, 'ip' => $ip, 'rango' => $rango, 'id_estructura' => $id_estructura, 'dependencia' => $dependencia,'nombre_dependencia'=>$nombre_dependencia,'arbol_id'=>$arbol_id]);
    ...
    public function actionNuevoUsuario(){
        ...
        $dependencias = dependenciasModel::select('DEPEN_ID','DEPEN_DESC')
                        ->where('DEPEN_ID','like','%211C04%')->orderBy('DEPEN_ID','DESC')
                        ->get();
    ...
    public function actionVerUsuario(){
        ...
        $dependencias= dependenciasModel::select('DEPEN_ID','DEPEN_DESC')                   
                       ->where('DEPEN_ID','like','%211C04%')
                       ->get();
        $usuarios    = usuarioModel::select('FOLIO','NOMBRE_COMPLETO','EMAIL','CVE_DEPENDENCIA','CVE_ARBOL',
                                            'LOGIN','PASSWORD','STATUS_1','STATUS_2')
    ...
    public function actionBorrarUsuario($id){
        ...
        /************ Elimina el usuario **************************************/
        $usuarios=usuarioModel::select('FOLIO','NOMBRE_COMPLETO','EMAIL','CVE_DEPENDENCIA','LOGIN','PASSWORD',
                                       'STATUS_1','STATUS_2')
                  ->where('FOLIO',$id);
        if($usuarios->count() <= 0)
