...
class padronController extends Controller
{
    ...
    public function actionBuscarPadron(Request $request)
    {
        ...
        $regservicios = regServicioModel::join('JP_CAT_RUBROS','JP_CAT_RUBROS.RUBRO_ID', '=', 
                                                               'JP_CAT_SERVICIOS.RUBRO_ID')
                        ->select('JP_CAT_SERVICIOS.SERVICIO_ID','JP_CAT_SERVICIOS.SERVICIO_DESC',
                                 'JP_CAT_RUBROS.RUBRO_DESC')
                        ->get();                          
        ...
        //$bio   = $request->get('bio');    
        $regpadron= regPadronModel::orderBy('IAP_ID', 'ASC')
        ...
        if($regpadron->count() <= 0){
            toastr()->error('No existen beneficiarios en padron.','Lo siento!',['positionClass' => 'toast-bottom-right']);
            //return redirect()->route('nuevoPadron');
        }            
        return view('sicinar.padron.verPadron', compact('nombre','usuario','regpadron','regentidades','regmunicipio','regperiodos','regiap','regservicios','regmeses','regdias'));
    }
    ...
    public function actionnuevoPadron(){
        ...
        $regservicios = regServicioModel::join('JP_CAT_RUBROS','JP_CAT_RUBROS.RUBRO_ID', '=', 
                        ...
                        ->select('JP_CAT_SERVICIOS.SERVICIO_ID','JP_CAT_SERVICIOS.SERVICIO_DESC',
                                 'JP_CAT_RUBROS.RUBRO_DESC')
                        ->orderBy('JP_CAT_SERVICIOS.SERVICIO_DESC','ASC')
                        ->orderBy('JP_CAT_RUBROS.RUBRO_DESC','ASC')
                        ->get();                             
        ...
        $regpadron    = regPadronModel::select('PERIODO_ID','PROGRAMA_ID','FOLIO','IAP_ID','PRIMER_APELLIDO',
                        ...
                        ->get();
        //dd($unidades);
        return view('sicinar.padron.nuevoPadron',compact('regmunicipio','regentidades','regiap','regperiodos','regpadron','regservicios','regmeses','regdias','nombre','usuario'));
    }
    ...
    public function actionAltanuevoPadron(Request $request){
        ...
        if($triada->count() >= 1)
            ...
            // *************** Validar curp ***********************************/
            $dupcurp = regPadronModel::where('CURP',substr(strtoupper(trim($request->curp)),0,17))
                       ->get();
            if($dupcurp->count() >= 1)
                ...
                $nuevoPadron->IAP_ID           = $request->iap_id;
                ...
                $nuevoPadron->CURP             = substr(strtoupper(trim($request->curp)),0,17);
    ...
    public function actionverPadron(){
        ...
        return view('sicinar.padron.verPadron',compact('nombre','usuario','regiap','regentidades', 'regmunicipio', 'regperiodos','regpadron','regservicios','regmeses','regdias'));
    ...
    public function actionEditarPadron($id){
        ...
        return view('sicinar.padron.editarPadron',compact('nombre','usuario','regiap','regentidades','regmunicipio','regperiodos','regpadron','regservicios','regmeses','regdias'));
    ...
    public function actionExportPadronPdf(){
        ...
        $pdf = PDF::loadView('sicinar.pdf.padronPdf', compact('nombre','usuario','regentidades','regmunicipio','regiap','regperiodos','regpadron','regservicios','regmeses','regdias'));
        //$options = new Options();
    ...
    public function actionPadronxEdo(){
        ...
        $regtotxedo=regPadronModel::join('JP_CAT_ENTIDADES_FED','JP_CAT_ENTIDADES_FED.ENTIDADFEDERATIVA_ID','=',
                                                                'JP_METADATO_PADRON.ENTIDAD_FED_ID')
                    ->selectRaw('COUNT(*) AS TOTALXEDO')
                    ->get();
        $regpadron =regPadronModel::join('JP_CAT_ENTIDADES_FED','JP_CAT_ENTIDADES_FED.ENTIDADFEDERATIVA_ID','=',
                                                                'JP_METADATO_PADRON.ENTIDAD_FED_ID')
                      ->selectRaw('JP_METADATO_PADRON.ENTIDAD_FED_ID, 
                                   JP_CAT_ENTIDADES_FED.ENTIDADFEDERATIVA_DESC AS ESTADO, COUNT(*) AS TOTAL')
                      ->groupBy('JP_METADATO_PADRON.ENTIDAD_FED_ID','JP_CAT_ENTIDADES_FED.ENTIDADFEDERATIVA_DESC')
                      ->orderBy('JP_METADATO_PADRON.ENTIDAD_FED_ID','asc')
                      ->get();
        //dd($procesos);
    ...
    public function actionPadronxMpio(){
        ...
        $regtotxmpio=regPadronModel::join('JP_CAT_MUNICIPIOS_SEDESEM',
                                     [['JP_CAT_MUNICIPIOS_SEDESEM.ENTIDADFEDERATIVAID','=',15],
                                      ['JP_CAT_MUNICIPIOS_SEDESEM.MUNICIPIOID','=','JP_METADATO_PADRON.MUNICIPIO_ID']
                                      ])
                         ->selectRaw('COUNT(*) AS TOTALXMPIO')
                               ->get();
        $regpadron=regPadronModel::join('JP_CAT_MUNICIPIOS_SEDESEM',
                                      [['JP_CAT_MUNICIPIOS_SEDESEM.ENTIDADFEDERATIVAID','=',15],
                                       ['JP_CAT_MUNICIPIOS_SEDESEM.MUNICIPIOID','=','JP_METADATO_PADRON.MUNICIPIO_ID']
                                      ])
                      ->selectRaw('JP_METADATO_PADRON.MUNICIPIO_ID, JP_CAT_MUNICIPIOS_SEDESEM.MUNICIPIONOMBRE AS MUNICIPIO,COUNT(*) AS TOTAL')
                        ->groupBy('JP_METADATO_PADRON.MUNICIPIO_ID', 'JP_CAT_MUNICIPIOS_SEDESEM.MUNICIPIONOMBRE')
                        ->orderBy('JP_METADATO_PADRON.MUNICIPIO_ID','asc')
                        ->get();
        //dd($procesos);
    ...
    public function actionPadronxServicio(){
        ...
        $regtotales=regPadronModel::join('JP_CAT_SERVICIOS','JP_CAT_SERVICIOS.SERVICIO_ID',  '=',
                                                            'JP_METADATO_PADRON.SERVICIO_ID')
                    ->selectRaw('COUNT(*) AS TOTAL')
                    ->get();
        $regpadron =regPadronModel::join('JP_CAT_SERVICIOS','JP_CAT_SERVICIOS.SERVICIO_ID','=',
                                                            'JP_METADATO_PADRON.SERVICIO_ID')
                    ->selectRaw('JP_METADATO_PADRON.SERVICIO_ID, 
                                 JP_CAT_SERVICIOS.SERVICIO_DESC AS SERVICIO, COUNT(*) AS TOTAL')
                    ->groupBy('JP_METADATO_PADRON.SERVICIO_ID','JP_CAT_SERVICIOS.SERVICIO_DESC')
                    ->orderBy('JP_METADATO_PADRON.SERVICIO_ID','asc')
                    ->get();
        //dd($regpadron);
